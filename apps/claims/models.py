...
class Claim(BaseModel):
    ...
    operating_hours = models.DecimalField(
        max_digits=10, decimal_places=2, verbose_name="Наработка, м/час"
    )
    failure_node = models.ForeignKey(
        "references.FailureNode", on_delete=models.PROTECT, verbose_name="Узел отказа"
    )
    failure_description = models.TextField(verbose_name="Описание отказа")
    restoration_method = models.ForeignKey(
        "references.RestorationMethod",
        on_delete=models.PROTECT,
        verbose_name="Способ восстановления",
    )
    spare_parts_used = models.TextField(
        blank=True, null=True, verbose_name="Используемые запасные части"
    )
