...
def add_maintenance(request, machine_pk):
    ...
    if not (request.user.is_superuser or
            (profile and profile.role == 'service' and profile.organization) or
            (profile and profile.role == 'manager')):
    ...
    return render(request, 'machines/maintenance_form.html', {
        'form': form, 'machine': machine, 'title': f'Добавить ТО | {machine.serial_number}'
    })
...
def add_claim(request, machine_pk):
    ...
    profile = getattr(request.user, 'profile', None)
    if not (request.user.is_superuser or
            (profile and profile.organization and profile.role in ['client', 'service', 'manager'])):
    ...
    return render(request, 'machines/claim_form.html', {
        'form': form, 'machine': machine, 'title': f'Создать рекламацию | {machine.serial_number}'
    })
